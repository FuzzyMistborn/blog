...
I did have a slight annoyance with SimpleLogin (that is not their fault): I want to actually use my root domain name for normal email, so I had to put SimpleLogin on a subdomain for email.  So my main email is `@example.com` and SimpleLogin is hosted at `@sl.example.com`.  Not the worst thing but it is ever so slightly annoying if I have to give out the email address and potentially explain that yes, there is a period in the domain name part.
# MXroute
